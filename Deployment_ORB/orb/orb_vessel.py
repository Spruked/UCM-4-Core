...
class OrbVessel:
    ...
    async def _observation_loop(self):
        """The ORB's eternal watch (now with SKG learning)"""
        while self.is_observing:
            try:
                ...
                # Every 30 seconds, ingest new observations into SKG
                if int(datetime.utcnow().timestamp()) % 30 == 0:
                    SKG_ENGINE.ingest_from_matrix(self.matrix)
                
                # Every 60 seconds, run edge detection
                if int(datetime.utcnow().timestamp()) % 60 == 0:
                    clutter = SKG_ENGINE.detect_clutter_edges()
                    if clutter:
                        print(f"[ORB-SKG] Detected {len(clutter)} clutter edges")
