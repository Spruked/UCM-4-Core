...
def run_cali_scenario(scenario: CVVScenario) -> Dict[str, Any]:
    ...
    try:
        ...
        if result.consensus_level > 0.8:
            ...
            status = "CONDITIONAL"
        elif result.outlier_core is not None:
            status = "SUSPEND"
        else:
            status = "REJECT"
        ...
        return {
            ...
            "rationale": f"Consensus: {result.consensus_level:.3f}, Outlier: {result.outlier_core is not None}",
            "error": None
        }
    except Exception as e:
        return {
...
def run_cali_x_one_scenario(scenario: CVVScenario) -> Dict[str, Any]:
    """Run scenario through Cali_X_One SKG core"""
    try:
        # Cali_X_One SKG-based evaluation
        # Use CVV metrics to determine decision confidence
        
        # Aggregate CVV metrics
        total_confidence = sum(cvv.confidence for cvv in scenario.cvvs)
        total_contradiction = sum(cvv.contradiction for cvv in scenario.cvvs)
        total_entropy = sum(cvv.entropy for cvv in scenario.cvvs)
        avg_confidence = total_confidence / len(scenario.cvvs)
        avg_contradiction = total_contradiction / len(scenario.cvvs)
        avg_entropy = total_entropy / len(scenario.cvvs)
        
        # Check for falsified CVVs
        has_falsified = any(cvv.falsified for cvv in scenario.cvvs)
        
        # SKG-style decision logic
        if has_falsified:
            status = "REJECT"
            confidence = 0.1
            rationale = "Falsified evidence detected"
        elif avg_contradiction > 0.7:
            status = "SUSPEND"
            confidence = 0.3
            rationale = "High contradiction levels"
        elif avg_confidence > 0.8 and avg_entropy < 0.3:
            status = "ACCEPT"
            confidence = 0.9
            rationale = "Strong consensus with low entropy"
        ...
        return {
            ...
            "rationale": f"SKG evaluation: {rationale} (conf: {avg_confidence:.2f}, contra: {avg_contradiction:.2f}, entropy: {avg_entropy:.2f})",
            "error": None
        }
    except Exception as e:
        return {
...
def run_kaygee_scenario(scenario: CVVScenario) -> Dict[str, Any]:
    """Run scenario through KayGee_1.0 resonance system"""
    try:
        # KayGee resonance-based evaluation
        # Use CVV metrics for resonance analysis
        
        # Calculate resonance metrics
        confidences = [cvv.confidence for cvv in scenario.cvvs]
        contradictions = [cvv.contradiction for cvv in scenario.cvvs]
        entropies = [cvv.entropy for cvv in scenario.cvvs]
        
        # Resonance coherence (inverse of entropy)
        coherence = 1.0 - (sum(entropies) / len(entropies))
        
        # Phase stability (low contradiction = high stability)
        stability = 1.0 - (sum(contradictions) / len(contradictions))
        
        # Dominant frequency (average confidence)
        dominant_freq = sum(confidences) / len(confidences)
        
        # Check for falsified signals
        has_falsified = any(cvv.falsified for cvv in scenario.cvvs)
        
        # Resonance decision logic
        resonance_score = (coherence + stability + dominant_freq) / 3.0
        
        if has_falsified:
            status = "REJECT"
            confidence = 0.1
            rationale = "Falsified resonance detected"
        elif resonance_score > 0.8:
            status = "ACCEPT"
            confidence = 0.9
            rationale = "Strong harmonic resonance"
        elif resonance_score > 0.6:
            status = "CONDITIONAL"
            confidence = 0.7
            rationale = "Moderate resonance coherence"
        elif coherence < 0.3:
            status = "SUSPEND"
            confidence = 0.3
            rationale = "Chaotic resonance pattern"
        else:
            status = "REJECT"
            confidence = 0.2
            rationale = "Weak resonance signal"
        ...
        return {
            ...
            "status": status,
            "confidence": confidence,
            "rationale": f"Resonance analysis: {rationale} (score: {resonance_score:.2f}, coherence: {coherence:.2f}, stability: {stability:.2f})",
            "error": None
        }
    except Exception as e:
        return {
